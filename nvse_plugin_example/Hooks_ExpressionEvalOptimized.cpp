...
bool ParseBytecode(kEvaluator::TokenList& tokenList, Expression::Expression* expr, UInt8* scriptData, UInt32 numChars, Script* script)
{
	char exprSubset[0x1000];
	char tokenStringBuf[0x1000];
...
double __fastcall Hook_Expression_Eval(Expression::Expression* expr, UInt32 EDX, UInt8* scriptData, TESObjectREFR* ref, TESObjectREFR* containingObj, Script* scriptObj, ScriptEventList* eventList, std::size_t numChars, bool aFalse)
{
	...
	for (auto iter = tokenList.tokens.Begin(); !iter.End() && expr->lastErrorID == 0; ++iter)
	{
		...
		{
			auto result = 0.0;
			auto opcodeOffset = token.value.opcodeOffset;
			if (GameFunctions::EvaluateCommandRef(&result, scriptObj->data, &opcodeOffset, ref, containingObj, scriptObj, eventList, false))
			{
